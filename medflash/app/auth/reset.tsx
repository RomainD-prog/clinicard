...
export default function ResetPasswordScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      // évite de re-run (Linking.useURL peut changer)
      ...
      try {
        ...
        const { error } = await setRecoverySessionFromUrl(initial);

        if (error) {
          Alert.alert("Lien invalide", error, [
            { text: "OK", onPress: () => router.replace("/auth/login") },
          ]);
          return;
        }
        ...
        Alert.alert("Erreur", e?.message ?? "Impossible de valider le lien.", [
          { text: "OK", onPress: () => router.replace("/auth/login") },
        ]);
      }
    })();
